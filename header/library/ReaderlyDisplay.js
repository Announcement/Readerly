...
var $ = require('jquery');
var coreCSSstr = require('./library/core-CSS');
var nouiCSSstr = require('./library/settings/noui-CSS');
//
...
var ReaderlyDisplay = function ReaderlyDisplay(timer, parentNode, settings) {
  var rDis = {};
  ...
  rDis.close = function () {
    ...
    rDis.hide();
    for (var trigi = 0; trigi < rDis._toTrigger.length; trigi++) {
      var obj = rDis._toTrigger[trigi];
  ...
  var keybinds = function keybinds(event) {
    switch (event.keyCode) {
      ...
      case 32:
        ...
        $(readerly).find('#__rdly_text_button').click();
        break;
  ...
  rDis.show = function () {
    $iframe.show();
    style.textContent = 'p, a, i, li, h1, h2, h3, h4, h5, h6, img, div.tx, div.tx1, div.tx2 :not(iframe):not(script) { filter: blur(2.5px); user-select:none; pointer-events:none; } body :not(iframe):not(script) { user-select:none; pointer-events:none; } #__rdly_iframe { user-select:none; }';
  ...
  rDis._init = function (parentNode) {
    if (!$('#__rdly_iframe')[0]) {
      rDis._addNodes(parentNode)._addEvents()
...
exports.default = ReaderlyDisplay;
// }));