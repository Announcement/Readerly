...
;(function (root, setupFactory) {
  // root is usually `window`
  if (typeof define === 'function' && define.amd) {
    // amd if possible
    // AMD. Register as an anonymous module.
    define(['jquery', 'franc', 'iso-639.json', '@knod/unfluff', '@knod/sbd'], function (jQuery, franc, langCodes, unfluff, sbd) {
      return root.Parser = setupFactory(jQuery, franc, langCodes, unfluff, sbd);
    });
...
})(undefined, function ($, franc, langCodes, unfluff, sbd) {
  /* (jQuery, {}, {}, {}, {}) -> Parser Setup Constructor */

  'use strict';

  var ParserSetup = function ParserSetup() {
    /* () -> ParserSetup
    *
    ...
    */
    ...
    rSup.cleanNode = function (node) {
      ...
      */
      ...
      for (var tagi = 0; tagi < toRemove.length; tagi++) {
        var tag = toRemove[tagi];
    ...
    rSup.detectLanguage = function (text) {
      ...
      */
      ...
      {
        whitelist: ['ara', 'bul', 'ces', 'dan', 'deu', 'ell', 'eng', 'spa', 'fin', 'fra', 'hun', 'ind', 'ita', 'kor', 'nob', 'nor', 'pol', 'por', 'rus', 'swe', 'tha', 'tur', 'zho']
      });
      if (lang === 'und') {
        lang = 'eng';
      }
