...
var $ = require('jquery');
var noUiSlider = require('@knod/nouislider');
var playbackCSSstr = require('./library/playback/playback-CSS');
...
function PlaybackUI(timer, coreDisplay) {
  ...
  var textContainerStr = '<div class="__rdly-flexible"><span id="__rdly-text-left" class="__rdly-text"></span><span id="__rdly-text-center" class="__rdly-text"></span><span id="__rdly-text-right" class="__rdly-text"></span></div>',
      indicatorStr = '<div id="__rdly_indicator" class="__rdly-center __rdly-flexible"><span>|</span></div>',
  ...
  */
  ...
  rPUI._showNewFragment = function (evnt, timer, fragment) {
    ...
    if (!whiteSpaceRegexp.test(chars)) {
      ...
      if (chars.includes(' ')) {
        ...
        textParts.middleText = chars;
        textParts.endText = '';
      } else if (chars.includes('document.getElementById') || chars.includes('createScene();')) {
        textParts.startText = '';
        textParts.middleText = chars;
        textParts.endText = '';
      ...
      if (noPunctuationLength >= 2) {
        ...
        while (textParts.middleText.match(nonCharRegEx) && textParts.endText.length > 0 && noPunctuationLength > 2) {
          rPUI._shiftCharacter(textParts);
        }
      }
  ...
  rPUI._init = function (coreDisplay) {
    ...
    // STYLES
    playbackCSSstr = playbackCSSstr + '@font-face { font-family: Clear Sans Light;' + 'src: url(' + fontPath + ');}';
