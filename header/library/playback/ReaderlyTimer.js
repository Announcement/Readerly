...
var ReaderlyTimer = function ReaderlyTimer(delayer) {
  /* ( {} ) -> ReaderlyTimer
         *
         */
  var rTim = {};
  ...
  var isLastWord = function isLastWord() {
    return rTim.getProgress() === 1;
  };
  var isFragmented = function isFragmented() {
    return rTim.lastWordFragmentCount() > 1;
  };
  var doneNoProlongation = function doneNoProlongation() {
    return isLastWord() && !isFragmented();
  };
  var doneWithProlongation = function doneWithProlongation() {
  ...
  var stopLoop = function stopLoop() {
    $(rTim).trigger('done', [rTim]);
    return !!rTim.stop();
  };
  var stoppedLoop = function stoppedLoop() {
  ...
  rTim._skipDirection = function (incrementors, frag) {
    ...
    if (!chars) {
      console.log('frag:', frag, '; chars:', chars, '; position:', rTim._queue.position);
    }
