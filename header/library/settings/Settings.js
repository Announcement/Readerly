...
var Settings = function Settings(storage, oldSettings) {
  var rSet = {};
  ...
  rSet.available = [
  // ==== delayer ====
  'wpm', '_baseDelay', 'slowStartDelay', 'sentenceDelay', 'otherPuncDelay', 'numericDelay', 'shortWordDelay', 'longWordDelay', 'halfSpeed',
  // 'wordLengthDelays',
  // ==== fragmentor/splitter ====
  'maxNumCharacters'];
  ...
  rSet.set = function (settingName, value) {
    ...
    if (settingName[0] !== '_') {
      ...
      if (rSet._debug) {
        console.log('The setting', settingName, 'has just been saved with the normalized value', val);
      }
    }
  ...
  rSet._init = function (oldSettings) {
    ...
    for (var key in _settings) {
      var val = oldSettings[key] || _settings[key];
      rSet.set(key, val);
    }
