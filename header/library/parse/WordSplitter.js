...
var WordSplitter = function WordSplitter(charsNode, settings) {
  ...
  */
  ...
  rSpt._makeCharsMap = function (chars, maxWithHyphen) {
    ...
    */
    ...
    for (var numi = 0; numi < evenly; numi++) {
  ...
  rSpt._splitWord = function (chars, maxChars) {
    ...
    if ((hasSpace || specialCharacterCount / chars.length > 0.2) && !chars.match(/([a-zA-Z]|[ά-ωΑ-ώ])[-]([a-zA-Z]|[ά-ωΑ-ώ])/g) || chars.match(/([a-zA-Z]|[ά-ωΑ-ώ])+[.]([a-zA-Z]|[ά-ωΑ-ώ])+[.]([a-zA-Z]|[ά-ωΑ-ώ])+[;]/g)) {
      return [chars];
    }
    ...
    var start = 0;
    for (var numi = 0; numi < splitMap.length; numi++) {
      var str = chars.slice(start, start + splitMap[numi]);
