...
var WordNav = function WordNav() {
  ...
  wNav.index = 0;wNav.position = [0, 0, 0], wNav.currentWord = null; // [ Str ]
  ...
  wNav.process = function (senteceArray, fragmentor) {
    if (!senteceArray) {
      console.error('WordNav needs dataz to .process(). You gave it dis:', senteceArray);
    }
    ...
    for (var senti = 0; senti < sentences.length; senti++) {
      var sentence = sentences[senti];
      for (var wordi = 0; wordi < sentence.length; wordi++) {
  ...
  wNav._stepSentence = function (sentenceChange) {
    ...
    if (sentenceChange > 0 && senti >= sentences.length - 1) {
      ...
      if (sentenceChange === -1 && wordi > 0) {} else {
  ...
  var signOf = function signOf(num) {
