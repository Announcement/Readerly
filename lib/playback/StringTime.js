...
(function (root, timeFactory) {  // root is usually `window`
	...
	*/
		...
		*/
			...
			var PeriodCheck = (str.match(/^[(]*[^]+[)]*(["]|[”]|[’])*([.]|[?]|[!]|[…])[']*(["]|[”]|[’])*[)]*$/g) || str.match(/^[(]*[ά-ωΑ-ώ]+[)]*(["]|[”]|[’])*([.]|[?]|[!]|[…]|[;])[']*(["]|[”]|[’])*[)]*$/g)) && !str.match(/^[(]*[^]*(Wham|Yahoo)[)]*([!])[']*(["]|[”]|[’])*[)]*$/g);

			var OtherPuncCheck = str.match(/^(["]|[”])*[(]*[^ ]{4,}[)]*(["]|[”])*([:]|[,])(["]|[”])*$/g);

			var NumericCheck1 = str.match(/^[0-9]+[%]*[,]*[)]*[:]*$/g) || str.match(/^[(]*(Mr|Ms|Dr|Sir|No|St|vs)[)]*[.][,]*$/g) || str.match(/^[(]*[a-zA-Zά-ωΑ-ώ][.][,]*$/g) || str.match(/^[(]*[a-zά-ωA-ZΑ-ώ][.][a-zά-ωA-ZΑ-ώ][)]*[.][,]*$/g) || str.match(/^[0-9]+[.]*[0-9]*(GHz|MHz|Khz)$/g) || str.match(/^[a-zά-ωA-ZΑ-ώ]{1,5}[.][a-zά-ωA-ZΑ-ώ]{1,5}[.][a-zά-ωA-ZΑ-ώ]{1,5}$/g);

      var NumericCheck2 = str.match(/^[(][a-zά-ωA-ZΑ-ώ]+[)]$/g) || str.match(/^[$][0-9]{1,3}[.][0-9]{1,3}$/g) || str.match(/^[&]$/g) || str.match(/^(Sept|Oct|Dec|Jan|Mar|Aug|Decem|Decemb|Nov|Novem|Novemb)[.]$/g) || str.match(/^[(][0-9]+([.]|[,])[0-9]+[a-zA-Zά-ωΑ-ώ][)]$/g) || str.match(/^[(]*[κ][)]*[.][,]*$/g) || str.match(/^["]*[a-zά-ωA-ZΑ-ώ]{1,10}[-][a-zά-ωA-ZΑ-ώ]{1,10}["]*[,]*$/g);

			var NumericCheck3 = str.match(/^["]*[a-zA-Zά-ωΑ-ώ]{4,}[:][a-zA-Zά-ωΑ-ώ]["]*$/g);

			var NumericCheck = NumericCheck1 || NumericCheck2 || NumericCheck3;

			var CodeCheck = !PeriodCheck && !OtherPuncCheck && !NumericCheck && !str.match(/^[(][a-zA-Zά-ωΑ-ώ]+$/g) && !str.match(/^[a-zA-Zά-ωΑ-ώ]+[)]$/g) && !str.match(/^[(][a-zA-Zά-ωΑ-ώ]+[)]$/g);

			var Guard = str.match(/^[^a-zA-Zά-ωΑ-ώ0-9 ]{0,1}[a-zA-Zά-ωΑ-ώ]+[/]*[a-zA-Zά-ωΑ-ώ]*[-]*[']*[s]*["]*$/g) || str.match(/^[^a-zA-Zά-ωΑ-ώ0-9 ]{0,1}[a-zA-Zά-ωΑ-ώ]+[-][^]+[-]$/g);
      ...
      if ( processed[ key ] && CodeCheck && str.length >= 6 && !Guard ) {
				delayModKey = toMultiplyBy[ key ];
        delay *= stm.orDefault( delayModKey ) + (0.50 * (str.length - 10));
			}
		...
		*/
			...
			var specialCharacters = chars.match(/[\x21-\x2F\x3A-\x40\x5B-\x60\x7B-\x7E]/g);
      var specialCharacterCount = specialCharacters != null ? specialCharacters.length : 0;
      var hasSpace = chars.includes(" ");
			result.hasPeriod = /[.!?]/.test(chars);
			result.isNumeric = /\d/.test(chars);

			result.isCode = (hasSpace || (specialCharacterCount / chars.length) > 0.20);
			result.isShort = (!result.isCode && !result.hasPeriod && chars.length <= shortLength);
			result.isLong  = (!result.isCode && !result.hasPeriod && chars.length >= longLength);
