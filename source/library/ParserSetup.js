...
;(function (root, setupFactory) {
  ...
  if (typeof define === 'function' && define.amd) {
    ...
    define(
      ['jquery', 'franc', 'iso-639.json', '@knod/unfluff', '@knod/sbd'],
      function (jQuery, franc, langCodes, unfluff, sbd) {
        return (root.Parser = setupFactory(
          jQuery,
          franc,
          langCodes,
          unfluff,
    ...
    module.exports = setupFactory(
      require('jquery'),
      require('franc'),
      require('./parse/iso-639.json'),
      require('@knod/unfluff'),
      require('@knod/sbd')
    )
  } else {
    ...
    // Browser globals
    console.warn(
      'If this isn\'t running off browserify, I\'m not sure how to get these libs in here.'
    )
    ...
    */
    ...
    rSup.findArticle = function (node, lang) {
      ...
      if (rSup.debug) {
        // Help non-coder devs identify some bugs
        console.log(
          '~~~parse debug~~~ article text identified (a string):',
          text
        )
      }
