...
(function () {
		...
		function splitCode(domNode) {
			return Array.from(domNode.childNodes).map((childNode) => {
				let splitMethod;
    ...
    var multisiteRule = function (var1, sitename, sitename2, var2, element) {
      var $page = $('html').clone();
      var pagex = $page[0];
      ...
      if ( var1.includes(sitename) && var3.includes(sitename2) ) {
		...
		var readSelectedText = function(){
		    ...
		    var junkElements = docFragment.querySelectorAll('h1, h2, h3, h4, h5, h6, #jump-to-nav, #siteSub, .infobox, [role=note], span.attribution, span.caption, div.adblocker-message, q.quote__content, table.wikitable, div.rellink, span.bullet, div.floatingad, span.mw-editsection, div.thumbcaption, div.magnify, div.toc');
		    if(junkElements.length){
			     for(var i = 0, l = junkElements.length; i < l; i++){
			         junkElements[i].parentNode.removeChild(junkElements[i]);
			     }
        ...
        var clonex = clonex.replace(/<div class=["]hiddenStructure[^]+<\/div>/g, '');
        var clonex = clonex.replace(/<a href=["]http:\/\/pages.email.bbc.com\/subscribe["] class=["]story-body__link["][^]+<\/a>/g, '');
      ...
      //node.innerHTML = html;
        ...
        var storage = clone1.replace(/<div class=["]hiddenStructure.+[\s]*.+[\s]*.+<\/div>/g, '');
        var storage = storage.replace(/<div class=["]col action tooltip hide["][^>]*>[\s]*.+[\s]*.+<\/div>/g, '');
        var storage = storage.replace(/<div class=["]exp action tooltip["][^>]*>[\s]*.+[\s]*.+<\/div>/g, '');
        var storage = storage.replace(/<div class=["]breadcrumbs clear["][^>]*><ul>[\s]+<li>.+[\s]*<li>.+[\s]*<li>.+[\s]*<li>.+[\s]*<\/ul><\/div>/g, '');
        ...
        var storage = storage.replace(/(?:[a-zA-Z]|[ά-ωΑ-ώ])[\s]+[.](?:^[a-zA-Z]|[ά-ωΑ-ώ])/g, '.');
